...
async function validateJWT(request) {
  ...
  try {
    ...
    const toBuf = s => { const b = atob(s.replace(/-/g,'+').replace(/_/g,'/')); return new Uint8Array(b.length).map((_, i) => b.charCodeAt(i)); };

    const dec = new TextDecoder();
    const header = JSON.parse(dec.decode(toBuf(headerB64)));
    const payload = JSON.parse(dec.decode(toBuf(payloadB64)));
    ...
    const valid = await crypto.subtle.verify(
      'RSASSA-PKCS1-v1_5', cryptoKey, toBuf(sigB64), message,
    );
