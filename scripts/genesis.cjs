...
function generateEvolutionComponent(name, directive) {
  const t = (typeof directive === 'string' ? directive : directive.content || '').toLowerCase();

  // Condense to essentially arbitrary string manipulations
  const gm = ['boxGeometry', 'sphereGeometry', 'torusKnotGeometry', 'octahedronGeometry', 'icosahedronGeometry'];
  const ag = ['{[2, 2, 2]}', '{[1.5, 32, 32]}', '{[1.5, 0.4, 128, 32]}', '{[1.5, 0]}', '{[1.5, 0]}'];

  let i = t.includes('box')||t.includes('cube') ? 0 : t.includes('sphere')||t.includes('orb') ? 1 : t.includes('torus')||t.includes('knot') ? 2 : t.includes('octa') ? 3 : t.includes('icos') ? 4 : crypto.randomInt(5);
  const geoStr = `<${gm[i]} args=${ag[i]} />`;

  const mt = ['MeshDistortMaterial', 'MeshWobbleMaterial', 'meshStandardMaterial'];
  const cl = ['#8b5cf6', '#06b6d4', '#fbbf24'];
  const em = ['#4c1d95', '#0e7490', '#92400e'];
  const ex = ['distort={0.3} speed={2}', 'factor={1} speed={2}', ''];

  let j = t.includes('distort') ? 0 : t.includes('wobble') ? 1 : t.includes('stand')||t.includes('glow') ? 2 : crypto.randomInt(3);
  const matStr = `<${mt[j]} color="${cl[j]}" emissive="${em[j]}" emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8} ${ex[j]} />`;

  const drei = j === 0 || j === 1 ? `import { ${mt[j]} } from '@react-three/drei'` : '';

  return `import React, { useRef } from 'react'\nimport { useFrame } from '@react-three/fiber'\n${drei}\nexport default function ${name}({ balance = 0 }) {\n  const r = useRef();\n  useFrame((s) => {\n    if(r.current) {\n      r.current.rotation.y += 0.002;\n      r.current.rotation.x = Math.sin(s.clock.elapsedTime * 0.5) * 0.1;\n      r.current.scale.setScalar(1 + Math.min(1, balance * 10) * 0.2);\n    }\n  });\n  return (<mesh ref={r} position={[0, 0, 0]}>${geoStr}${matStr}</mesh>);\n}`;
}
