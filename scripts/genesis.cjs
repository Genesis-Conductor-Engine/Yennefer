...
function generateEvolutionComponent(name, directive) {
  ...
  // Ensure correct imports based on material chosen
  let dreiImports = [];
  if (material.includes('MeshDistortMaterial')) dreiImports.push('MeshDistortMaterial');
  if (material.includes('MeshWobbleMaterial')) dreiImports.push('MeshWobbleMaterial');

  const importStatement = dreiImports.length > 0
    ? `import { ${dreiImports.join(', ')} } from '@react-three/drei'`
    : '';

  // Randomize position a bit so multiple mutations don't completely overlap
  const posX = (Math.random() * 4 - 2).toFixed(2);
  const posY = (Math.random() * 4 - 2).toFixed(2);
  const posZ = (Math.random() * 4 - 2).toFixed(2);
...
// Execute loop if GENESIS_LOOP is set, otherwise run once
if (process.env.GENESIS_LOOP === 'true') {
  (async function loop() {
    while (true) {
      await genesis();
      // Wait for 1 minute before next cycle to prevent spamming
