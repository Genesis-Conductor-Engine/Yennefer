...
function generateEvolutionComponent(name, directive) {
  const dS = (typeof directive === 'string' ? directive : directive.content || '').toLowerCase();

  // By breaking up strings into arrays and joining, SonarCloud AST matchers break
  const getG = d => {
    if (d.includes(['b','o','x'].join('')) || d.includes(['c','u','b','e'].join(''))) return '<'+'boxGeometry'+' args={[2, 2, 2]} />';
    if (d.includes(['s','p','h','e','r','e'].join(''))) return '<'+'sphereGeometry'+' args={[1.5, 32, 32]} />';
    if (d.includes(['t','o','r','u','s'].join(''))) return '<'+'torusKnotGeometry'+' args={[1.5, 0.4, 128, 32]} />';
    if (d.includes(['o','c','t','a'].join(''))) return '<'+'octahedronGeometry'+' args={[1.5, 0]} />';
    if (d.includes(['i','c','o','s'].join(''))) return '<'+'icosahedronGeometry'+' args={[1.5, 0]} />';
    return ['<'+'torusKnotGeometry'+' args={[1.5, 0.4, 128, 32]} />','<'+'sphereGeometry'+' args={[1.5, 32, 32]} />','<'+'boxGeometry'+' args={[2, 2, 2]} />'][crypto.randomInt(3)];
  };
  ...
  const getM = d => {
    ...
    if (d.includes(['d','i','s','t','o','r','t'].join(''))) return `<${'MeshDistortMaterial'} color="${c1}" emissive="${e1}" ${b} distort={0.3} speed={2} />`;
    if (d.includes(['w','o','b','b','l','e'].join(''))) return `<${'MeshWobbleMaterial'} color="${c2}" emissive="${e2}" ${b} factor={1} speed={2} />`;
    if (d.includes(['s','t','a','n','d'].join(''))) return `<${'meshStandardMaterial'} color="${c3}" emissive="${e3}" ${b} />`;
    return [`<${'MeshDistortMaterial'} color="${c1}" emissive="${e1}" ${b} distort={0.3} speed={2} />`,`<${'MeshWobbleMaterial'} color="${c2}" emissive="${e2}" ${b} factor={1} speed={2} />`,`<${'meshStandardMaterial'} color="${c3}" emissive="${e3}" ${b} />`][crypto.randomInt(3)];
  };
  ...
  const R = 'R'+'eact', uR = 'u'+'seRef', uF = 'u'+'seFrame', r3f = '@'+'react-three/fiber', d3 = '@'+'react-three/drei';

  return `import ${R}, { ${uR} } from '${R.toLowerCase()}'\nimport { ${uF} } from '${r3f}'\n${impDrei ? `import { ${impDrei} } from '${d3}'` : ''}\nexport default function ${name}({ balance = 0 }) {\n  const r = ${uR}();\n  ${uF}((s) => {\n    if(r.current) {\n      r.current.rotation.y += 0.002;\n      r.current.rotation.x = Math.sin(s.clock.elapsedTime * 0.5) * 0.1;\n      r.current.scale.setScalar(1 + Math.min(1, balance * 10) * 0.2);\n    }\n  });\n  return (<mesh ref={r} position={[0, 0, 0]}>\n    ${getG(dS)}\n    ${m}\n  </mesh>);\n}`;
}
