...
async function dispatchTheBuilder(directive) {
  ...
  let baseName = directive.replace(/[^a-zA-Z]/g, '').slice(0, 15) || 'Mutation';
  // Capitalize first letter
  baseName = baseName.charAt(0).toUpperCase() + baseName.slice(1);
  const componentName = `${baseName}${Date.now()}`;
  
  // Generate a React Three Fiber component template using Project Genie simulator
  const componentCode = generateEvolutionComponent(directive, componentName);
