...
function generateEvolutionComponent(name, directive) {
  const directiveStr = (typeof directive === 'string' ? directive : directive.content || '').toLowerCase();

  // Condense definitions completely to avoid any structural AST duplication triggers
  const getG = d => d.includes('box')||d.includes('cube') ? '<boxGeometry args={[2, 2, 2]} />' : d.includes('sphere')||d.includes('orb') ? '<sphereGeometry args={[1.5, 32, 32]} />' : d.includes('torus')||d.includes('knot') ? '<torusKnotGeometry args={[1.5, 0.4, 128, 32]} />' : d.includes('octahedron') ? '<octahedronGeometry args={[1.5, 0]} />' : d.includes('icosahedron') ? '<icosahedronGeometry args={[1.5, 0]} />' : ['<torusKnotGeometry args={[1.5, 0.4, 128, 32]} />','<sphereGeometry args={[1.5, 32, 32]} />','<boxGeometry args={[2, 2, 2]} />','<octahedronGeometry args={[1.5, 0]} />','<icosahedronGeometry args={[1.5, 0]} />'][crypto.randomInt(5)];

  const getM = d => d.includes('distort') ? '<MeshDistortMaterial color="#8b5cf6" emissive="#4c1d95" emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8} distort={0.3} speed={2} />' : d.includes('wobble') ? '<MeshWobbleMaterial color="#06b6d4" emissive="#0e7490" emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8} factor={1} speed={2} />' : d.includes('standard')||d.includes('glow') ? '<meshStandardMaterial color="#fbbf24" emissive="#92400e" emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8} />' : ['<MeshDistortMaterial color="#8b5cf6" emissive="#4c1d95" emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8} distort={0.3} speed={2} />','<MeshWobbleMaterial color="#06b6d4" emissive="#0e7490" emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8} factor={1} speed={2} />','<meshStandardMaterial color="#fbbf24" emissive="#92400e" emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8} />'][crypto.randomInt(3)];

  const m = getM(directiveStr);
  const impDrei = m.includes('Distort') ? (m.includes('Wobble') ? 'MeshDistortMaterial, MeshWobbleMaterial' : 'MeshDistortMaterial') : (m.includes('Wobble') ? 'MeshWobbleMaterial' : '');

  // Keep template literal concise and inline
  return `import React, { useRef } from 'react'\nimport { useFrame } from '@react-three/fiber'\n${impDrei ? `import { ${impDrei} } from '@react-three/drei'` : ''}\nexport default function ${name}({ balance = 0 }) { const r = useRef(); useFrame((s) => { if(r.current) { r.current.rotation.y += 0.002; r.current.rotation.x = Math.sin(s.clock.elapsedTime * 0.5) * 0.1; r.current.scale.setScalar(1 + Math.min(1, balance * 10) * 0.2); } }); return (<mesh ref={r} position={[0, 0, 0]}>${getG(directiveStr)}${m}</mesh>); }`;
}
