...
function generateEvolutionComponent(name, directive) {
  ...
  const materials = [
    { type: 'drei', name: 'MeshDistortMaterial', props: 'distort={0.4} speed={2} roughness={0.2} metalness={0.8}' },
    { type: 'drei', name: 'MeshWobbleMaterial', props: 'factor={1} speed={1} roughness={0.2} metalness={0.8}' },
    { type: 'native', name: 'meshStandardMaterial', props: 'metalness={0.6} roughness={0.2}' },
    { type: 'native', name: 'meshPhysicalMaterial', props: 'metalness={0.1} roughness={0.1} clearcoat={1}' }
  ];

  const colors = ["#8b5cf6", "#ec4899", "#10b981", "#3b82f6", "#f59e0b", "#ef4444", "#6366f1"];

  const geo = geometries[Math.floor(Math.random() * geometries.length)];
  const mat = materials[Math.floor(Math.random() * materials.length)];
  const col = colors[Math.floor(Math.random() * colors.length)];

  const importLine = mat.type === 'drei'
    ? `import { ${mat.name} } from '@react-three/drei'`
    : `// Native material used`;
...
if (process.env.GENESIS_LOOP === 'true') {
  console.log("♾️  Running in Continuous Genesis Mode");
  genesis();
  setInterval(genesis, 60000);
} else {
  genesis();
}
