...
const { generateEvolutionComponent } = require('./lib/genie.cjs');
...
async function consultTheVisionary(state) {
  ...
  if (process.env.FORCE_MUTATION === 'true') {
    console.log("   ⚠️ FORCE_MUTATION enabled. Bypassing financial checks.");
    return {
      type: "MUTATE",
      content: "Forced mutation via Project Genie integration protocol."
    };
  }
...
async function dispatchTheBuilder(directive) {
  ...
  // Use a random name generator for variety if force mutation
  let componentName;
  if (process.env.FORCE_MUTATION === 'true') {
     const adjectives = ['Cosmic', 'Quantum', 'Stellar', 'Void', 'Nebula', 'Cyber'];
     const nouns = ['Shard', 'Core', 'Vortex', 'Pulse', 'Matrix', 'Flux'];
     componentName = adjectives[Math.floor(Math.random() * adjectives.length)] +
                     nouns[Math.floor(Math.random() * nouns.length)] +
                     Math.floor(Math.random() * 1000);
  } else {
     componentName = directive.replace(/[^a-zA-Z]/g, '').slice(0, 20) || 'Mutation';
  }
  
  // Generate a React Three Fiber component template using Project Genie logic
