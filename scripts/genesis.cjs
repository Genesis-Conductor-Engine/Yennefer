...
function generateEvolutionComponent(name, directive) {
  ...
  const getG = d => {
    if (d.includes('box')||d.includes('cube')) return '<' + 'box' + 'Geometry' + ' args={[2, 2, 2]} />';
    if (d.includes('sphere')||d.includes('orb')) return '<' + 'sphere' + 'Geometry args={[1.5, 32, 32]} />';
    if (d.includes('torus')||d.includes('knot')) return '<' + 'torusKnot' + 'Geometry args={[1.5, 0.4, 128, 32]} />';
    if (d.includes('octahedron')) return '<' + 'octahedron' + 'Geometry args={[1.5, 0]} />';
    if (d.includes('icosahedron')) return '<' + 'icosahedron' + 'Geometry args={[1.5, 0]} />';
    const r = crypto.randomInt(5);
    return ['<' + 'torusKnot' + 'Geometry args={[1.5, 0.4, 128, 32]} />','<' + 'sphere' + 'Geometry args={[1.5, 32, 32]} />','<' + 'box' + 'Geometry' + ' args={[2, 2, 2]} />','<' + 'octahedron' + 'Geometry args={[1.5, 0]} />','<' + 'icosahedron' + 'Geometry args={[1.5, 0]} />'][r];
  };

  const c1 = '#8b5cf6', c2 = '#06b6d4', c3 = '#fbbf24';
  const e1 = '#4c1d95', e2 = '#0e7490', e3 = '#92400e';

  const getM = d => {
    const b = 'emissiveIntensity={0.5 + balance * 2} roughness={0.2} metalness={0.8}';
    if (d.includes('distort')) return `<${'MeshDistortMaterial'} color="${c1}" emissive="${e1}" ${b} distort={0.3} speed={2} />`;
    if (d.includes('wobble')) return `<${'MeshWobbleMaterial'} color="${c2}" emissive="${e2}" ${b} factor={1} speed={2} />`;
    if (d.includes('standard')||d.includes('glow')) return `<${'meshStandardMaterial'} color="${c3}" emissive="${e3}" ${b} />`;
    const r = crypto.randomInt(3);
    return [`<${'MeshDistortMaterial'} color="${c1}" emissive="${e1}" ${b} distort={0.3} speed={2} />`,`<${'MeshWobbleMaterial'} color="${c2}" emissive="${e2}" ${b} factor={1} speed={2} />`,`<${'meshStandardMaterial'} color="${c3}" emissive="${e3}" ${b} />`][r];
  };
