...
function generateEvolutionComponent(name, directive) {
  ...
  const geoMap = [
    { keys: ["box", "cube"], idx: 2 },
    { keys: ["sphere", "orb"], idx: 1 },
    { keys: ["torus", "knot"], idx: 0 },
    { keys: ["octahedron"], idx: 3 },
    { keys: ["icosahedron"], idx: 4 }
  ];
  const matchedGeo = geoMap.find(m => m.keys.some(k => lowerDirective.includes(k)));
  const geometry = matchedGeo ? geometries[matchedGeo.idx] : geometries[crypto.randomInt(geometries.length)];
  ...
  const matMap = [
    { keys: ["distort"], idx: 0 },
    { keys: ["wobble"], idx: 1 },
    { keys: ["standard", "glow"], idx: 2 }
  ];
  const matchedMat = matMap.find(m => m.keys.some(k => lowerDirective.includes(k)));
  const material = matchedMat ? materials[matchedMat.idx] : materials[crypto.randomInt(materials.length)];
  ...
  const importedDrei = imports.length > 0 ? `import { ${imports.join(', ')} } from '@react-three/drei'` : '';
