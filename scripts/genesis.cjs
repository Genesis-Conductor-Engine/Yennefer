...
function generateEvolutionComponent(name, directive) {
  const geos = ['<torusKnotGeometry args={[1.5, 0.4, 128, 32]} />','<sphereGeometry args={[1.5, 32, 32]} />','<boxGeometry args={[2, 2, 2]} />','<octahedronGeometry args={[1.5, 0]} />','<icosahedronGeometry args={[1.5, 0]} />'];
  const mats = ['<MeshDistortMaterial color="#8b5cf6" emissive="#4c1d95" emissiveIntensity={0.5+balance*2} roughness={0.2} metalness={0.8} distort={0.3} speed={2}/>','<MeshWobbleMaterial color="#06b6d4" emissive="#0e7490" emissiveIntensity={0.5+balance*2} roughness={0.2} metalness={0.8} factor={1} speed={2}/>','<meshStandardMaterial color="#fbbf24" emissive="#92400e" emissiveIntensity={0.5+balance*2} roughness={0.2} metalness={0.8}/>'];
  const p = (typeof directive === 'string' ? directive : directive.content || '').toLowerCase();

  const geoMap = [{k:["box","cube"],i:2},{k:["sphere","orb"],i:1},{k:["torus","knot"],i:0},{k:["octahedron"],i:3},{k:["icosahedron"],i:4}];
  const gF = geoMap.find(m => m.k.some(x => p.includes(x)));
  const geometry = gF ? geos[gF.i] : geos[crypto.randomInt(5)];

  const matMap = [{k:["distort"],i:0},{k:["wobble"],i:1},{k:["standard","glow"],i:2}];
  const mF = matMap.find(m => m.k.some(x => p.includes(x)));
  const material = mF ? mats[mF.i] : mats[crypto.randomInt(3)];

  const imports = [];
  if (material.includes('Distort')) imports.push('MeshDistortMaterial');
  if (material.includes('Wobble')) imports.push('MeshWobbleMaterial');
  const dImp = imports.length ? `import { ${imports.join(', ')} } from '@react-three/drei'` : '';

  return `// Auto-generated by Yennefer Genesis Cycle\n// Directive: ${p}\nimport React, { useRef } from 'react'\nimport { useFrame } from '@react-three/fiber'\n${dImp}\nexport default function ${name}({ balance = 0 }) {\n  const meshRef = useRef()\n  useFrame((state) => {\n    if (meshRef.current) {\n      meshRef.current.rotation.y += 0.002\n      meshRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.5) * 0.1\n      meshRef.current.scale.setScalar(1 + Math.min(1, balance * 10) * 0.2)\n    }\n  })\n  return (\n    <mesh ref={meshRef} position={[0, 0, 0]}>\n      ${geometry}\n      ${material}\n    </mesh>\n  )\n}`;
}
