...
async function invokeTheScribe(task, content, state) {
  ...
  if (task === 'JOURNAL') {
    ...
    const logsDir = path.dirname(PATHS.journal);
...
// Generate React Three Fiber component code simulating Project Genie
function generateEvolutionComponent(name, directive) {
  ...
  const materials = [
    'MeshDistortMaterial',
    'MeshWobbleMaterial',
    'meshStandardMaterial' // not from drei
  ];
  ...
  if (materialType === 'MeshDistortMaterial') {
    materialCode = `<MeshDistortMaterial
        color="#8b5cf6"
        emissive="#4c1d95"
        ...
        speed={2}
      />`;
    dreiImport = `import { MeshDistortMaterial } from '@react-three/drei'`;
  } else if (materialType === 'MeshWobbleMaterial') {
    materialCode = `<MeshWobbleMaterial
        color="#10b981"
        emissive="#047857"
        emissiveIntensity={0.5 + balance * 2}
        roughness={0.2}
        ...
        speed={2}
      />`;
    dreiImport = `import { MeshWobbleMaterial } from '@react-three/drei'`;
  } else {
    materialCode = `<meshStandardMaterial
        color="#f59e0b"
        emissive="#d97706"
        emissiveIntensity={0.5 + balance * 2}
        roughness={0.2}
...
export default function ${name}({ balance = 0 }) {
  ...
  return (
    <mesh ref={meshRef} position={[${px.toFixed(2)}, ${py.toFixed(2)}, ${pz.toFixed(2)}]}>
      ${geometry}
      ${materialCode}
    </mesh>
  )
...
if (process.env.GENESIS_LOOP === 'true') {
  console.log("Starting Genesis loop...");
  // Run once immediately, then every 30 seconds
  genesis();
  setInterval(genesis, 30000);
} else {
  genesis();
}
