...
const b64H = 'Ly8gQXV0by1nZW5lcmF0ZWQgYnkgWWVubmVmZXIgR2VuZXNpcyBDeWNsZQppbXBvcnQgUmVhY3QsIHsgdXNlUmVmIH0gZnJvbSAncmVhY3QnCmltcG9ydCB7IHVzZUZyYW1lIH0gZnJvbSAnQHJlYWN0LXRocmVlL2ZpYmVyJw==';
const b64B = '';
const b64F = 'ICAgIDwvbWVzaD4KICApCn0=';
const parseB64 = s => Buffer.from(s, 'base64').toString('utf8');

const generateEvolutionComponent=(n,d)=>{
  const p=(typeof d==='string'?d:d.content||'').toLowerCase();
  const gM=['<torusKnotGeometry args={[1.5, 0.4, 128, 32]} />','<sphereGeometry args={[1.5, 32, 32]} />','<boxGeometry args={[2, 2, 2]} />','<octahedronGeometry args={[1.5, 0]} />','<icosahedronGeometry args={[1.5, 0]} />'];
  const mM=['<MeshDistortMaterial color="#8b5cf6" emissive="#4c1d95" emissiveIntensity={0.5+balance*2} roughness={0.2} metalness={0.8} distort={0.3} speed={2}/>','<MeshWobbleMaterial color="#06b6d4" emissive="#0e7490" emissiveIntensity={0.5+balance*2} roughness={0.2} metalness={0.8} factor={1} speed={2}/>','<meshStandardMaterial color="#fbbf24" emissive="#92400e" emissiveIntensity={0.5+balance*2} roughness={0.2} metalness={0.8}/>'];
  const gI=p.includes("box")||p.includes("cube")?2:p.includes("sphere")||p.includes("orb")?1:p.includes("torus")||p.includes("knot")?0:p.includes("octahedron")?3:p.includes("icosahedron")?4:crypto.randomInt(5);
  const mI=p.includes("distort")?0:p.includes("wobble")?1:p.includes("standard")||p.includes("glow")?2:crypto.randomInt(3);
  const imp=mM[mI].includes('Distort')?['MeshDistortMaterial']:mM[mI].includes('Wobble')?['MeshWobbleMaterial']:[];
  return `${parseB64(b64H)}\n${imp.length?`import { ${imp.join(', ')} } from '@react-three/drei'`:''}\nexport default function ${n}({ balance = 0 } ${parseB64(b64B)} ${gM[gI]} ${mM[mI]} ${parseB64(b64F)}`;
};
